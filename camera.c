...
/* USER CODE BEGIN PV */
	...
	int pwm_l = 0,pwm_r = 0;
	double err_l = 0,last_err_l = 0,err_sum_l = 0,err_r = 0,err_sum_r = 0,last_err_r = 0;
	double kp_l = 15.5,ki_l = 0.455,kd_l = 0.125,rp_l = 0,ri_l = 0,rd_l = 0;//左轮
	double kp_r = 15.5,ki_r = 0.49,kd_r = 0.15,rp_r = 0,ri_r = 0,rd_r = 0;//右轮
	...
	double bias = 0,last_bias = 0;
	double position_kp = 8,position_kd = 400,position_rp = 0,position_rd = 0,last_rd = 0,position_alpha = 0.2,val_l = 0,val_r = 0,val_m = 0,km = 5;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim == &htim5)
	{
		...
		//			//归一化
		//			val_l = (double)(inductance_val[0] - val_min_l) / (val_max_l - val_min_l);
		//			val_m = (double)(inductance_val[1] - val_min_m) / (val_max_m - val_min_m);
		//			val_r = (double)(inductance_val[2] - val_min_r) / (val_max_r - val_min_r);
		...
		//		}
			...
			if(delta_speed > 200)delta_speed = 200;
			if(delta_speed < -200)delta_speed  = -200;
			last_bias = bias;
			last_rd = position_rd;
	...
	//		else
	//		{
	//			ref_speed_l = ref_speed_l - delta_speed * 0.1;
	//		}
	//		if(bias > 0)
	//		{
	//			ref_speed_l = ref_speed_l - delta_speed / 2;
	...
	//			ref_speed_l = ref_speed_l - delta_speed / 2;
	//		}
			if(ref_speed_l > 650) ref_speed_l = 650;
			else if(ref_speed_l < 0)ref_speed_l = 0;
			err_l = ref_speed_l - measureval_l;
			err_sum_l += err_l;
			if (err_sum_l > 80000) err_sum_l = 80000;
			if (err_sum_l < -80000) err_sum_l = -80000;
			rp_l = kp_l * err_l;
			ri_l = ki_l * err_sum_l;
	...
	//		else
	//		{
	//			ref_speed_r = ref_speed_r + delta_speed * 0.1;
	//		}
	//		if(bias < 0)
	//		{
	//			ref_speed_r = ref_speed_r + delta_speed / 2;
	...
	//			ref_speed_r = ref_speed_r + delta_speed / 2;
	//		}
			if(ref_speed_r > 650) ref_speed_r = 650;
			else if(ref_speed_r < 0)ref_speed_r = 0;
			err_r = ref_speed_r - measureval_r;
			err_sum_r += err_r;
			if (err_sum_r > 80000) err_sum_r = 80000;
			if (err_sum_r < -80000) err_sum_r = -80000;
			rp_r = kp_r * err_r;
			ri_r = ki_r * err_sum_r;
